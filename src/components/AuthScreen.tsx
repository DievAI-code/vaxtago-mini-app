...
const FEATURES = [
  { icon: <Shield className="w-5 h-5" />, title: "Проверенные вакансии" },
  { icon: <Shield className="w-5 h-5" />, title: "Проверка работодателей" },
  { icon: <Bot className="w-5 h-5" />, title: "AI-помощник" },
  { icon: <Languages className="w-5 h-5" />, title: "Перевод документов" },
  { icon: <FileText className="w-5 h-5" />, title: "Распознавание документов" },
  { icon: <MapPin className="w-5 h-5" />, title: "Навигация" },
];
            ...
            {step === "phone" ? (
              ...
              >
                <div className="text-center mb-8">
                  ...
                  <h1 className="text-2xl font-bold text-white mb-2">Добро пожаловать в VaxtaGo</h1>
                  <p className="text-sm text-white/70">Авторизуйтесь, чтобы получить доступ ко всем возможностям приложения.</p>
                </div>
                ...
                <div className="space-y-4">
                  <div className="relative">
                    ...
                    {showCountries && (
                      <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        className="absolute top-full left-0 right-0 mt-2 bg-slate-800 rounded-2xl border border-white/20 shadow-xl z-10"
                      >
                        {COUNTRIES.map((c) => (
                          <button
                            key={c.code}
                            ...
                            className="w-full flex items-center gap-3 px-4 py-3 text-white hover:bg-white/10 transition-colors first:rounded-t-2xl last:rounded-b-2xl"
                          >
                  ...
                  <motion.button
                    ...
                    className="w-full py-4 rounded-2xl bg-gradient-to-r from-blue-600 to-purple-500 text-white font-semibold text-lg disabled:opacity-50 disabled:cursor-not-allowed hover:shadow-lg transition-all"
                  >
                    Продолжить
                  </motion.button>
                </div>
              ...
              >
                <div className="text-center mb-8">
                  ...
                  <p className="text-sm text-white/70">Мы отправили SMS-код на номер {country.code} {phone}</p>
                </div>
                ...
                <div className="space-y-4">
                  <input
                    ...
                    placeholder="••••"
                    maxLength={6}
                    className="w-full text-center text-3xl font-bold tracking-widest bg-white/10 rounded-2xl p-4 border border-white/20 text-white placeholder-white/30 outline-none focus:border-blue-400"
                  />
