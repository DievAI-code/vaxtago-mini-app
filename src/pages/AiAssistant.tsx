...
import { Send, Paperclip, Mic, Image as ImageIcon, Copy, Plus, Trash2, Download } from "lucide-react";
...
function renderMarkdown(text: string) {
  ...
  return escaped
    .replace(/```([\s\S]*?)```/g, '<pre class="bg-slate-800 rounded-lg p-2 my-1 overflow-x-auto text-xs">$1</pre>')
...
export default function AiAssistant() {
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-[#0F172A] text-white" style={{ paddingBottom: "env(safe-area-inset-bottom)" }}>
      <Header title="AI Помощник" />
      
      <div className="flex-shrink-0 flex items-center gap-2 px-4 py-2 border-b border-slate-800">
        <button onClick={clearHistory} className="p-2 rounded-xl text-slate-400 hover:bg-slate-800 transition" aria-label="Clear"><Trash2 size={16} /></button>
        <button onClick={exportChat} className="p-2 rounded-xl text-slate-400 hover:bg-slate-800 transition" aria-label="Export"><Download size={16} /></button>
        <div className="flex-1" />
        <span className="text-xs text-[#14B8A6] flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-[#14B8A6] inline-block" /> онлайн</span>
      </div>
      ...
      <div ref={messagesContainerRef} className="flex-1 min-h-0 overflow-y-auto px-4 py-4 space-y-4">
        <AnimatePresence initial={false}>
          {messages.map((m) => (
            <motion.div key={m.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className={`flex items-end gap-2 ${m.role === "user" ? "flex-row-reverse" : ""}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${m.role === "user" ? "bg-slate-600" : "bg-gradient-to-br from-[#2563EB] to-[#14B8A6]"}`}>
                {m.role === "user" ? <span className="text-xs">Вы</span> : <VaxtaGoLogo size={20} />}
              </div>
              <div className={`group max-w-[78%] rounded-2xl px-4 py-2.5 text-sm shadow-sm ${m.role === "user" ? "bg-[#2563EB] text-white rounded-br-md" : "bg-slate-800 text-slate-100 rounded-bl-md"}`}>
        ...
        {loading && (
          <div className="flex items-end gap-2">
            ...
            <div className="bg-slate-800 rounded-2xl rounded-bl-md px-4 py-3 shadow-sm"><TypingDots /></div>
          </div>
        )}
      </div>
      ...
      <div className="flex-shrink-0 px-4 py-2">
        <div className="flex gap-2 overflow-x-auto pb-2">
          {QUICK.map((q) => (
            <button key={q.label} onClick={() => handleSend(q.q)} className="px-3 py-1.5 rounded-full bg-slate-800/50 border border-slate-700 text-xs whitespace-nowrap hover:bg-slate-700/50 transition">{q.label}</button>
          ))}
        </div>
      </div>

      <div className="flex-shrink-0 p-3 bg-[#0F172A] border-t border-slate-800" style={{ paddingBottom: "calc(0.75rem + env(safe-area-inset-bottom))" }}>
        <div className="flex items-end gap-2">
          <button onClick={() => fileRef.current?.click()} className="p-2.5 rounded-full text-slate-400 hover:bg-slate-800 transition" aria-label="Attach"><Paperclip size={20} /></button>
          ...
          <button onClick={() => fileRef.current?.click()} className="p-2.5 rounded-full text-slate-400 hover:bg-slate-800 transition" aria-label="Photo"><ImageIcon size={20} /></button>
          <button className="p-2.5 rounded-full text-slate-400 hover:bg-slate-800 transition" aria-label="Voice"><Mic size={20} /></button>
          <div className="flex-1 relative">
            <textarea value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={onKey} rows={1} placeholder={t("chat_ph")} disabled={loading} className="w-full resize-none max-h-32 px-4 py-3 pr-2 rounded-2xl bg-slate-800 border border-slate-700 outline-none text-sm text-white placeholder-slate-400 disabled:opacity-50" />
          </div>
